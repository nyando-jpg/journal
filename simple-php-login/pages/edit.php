...
function has_local_image_path_for_edit(string $html): bool
{
...
try {
    ...
    $articleStmt = $pdo->prepare('SELECT * FROM journal_info WHERE id = :id LIMIT 1');
    $articleStmt->bindValue(':id', $articleId, PDO::PARAM_INT);
    $articleStmt->execute();
    ...
    $idCategorie = (int) $article['id_categorie'];
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        if ($titre === '' || $details === '' || $idCategorie <= 0) {
            ...
            if (!$existsStmt->fetchColumn()) {
                $errorCode = 'category';
            } elseif (has_local_image_path_for_edit($details)) {
                $errorCode = 'local_image';
            } else {
                $updateStmt = $pdo->prepare('UPDATE journal_info SET id_admin = :id_admin, id_categorie = :id_categorie, titre = :titre, details = :details WHERE id = :id');
                $updateStmt->bindValue(':id_admin', (int) $_SESSION['user_id'], PDO::PARAM_INT);
                $updateStmt->bindValue(':id_categorie', $idCategorie, PDO::PARAM_INT);
                $updateStmt->bindValue(':titre', $titre);
                $updateStmt->bindValue(':details', $details);
...
<body>
    ...
    <main>
        ...
        <p class="page-subtitle">Renseignez les informations ci-dessous pour mettre a jour l'article.</p>
